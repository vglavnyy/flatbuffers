...
CheckedError Parser::ParseEnumFromString(const Type &type,
  ...
  *result = IsUnsigned(base_type) ? NumToString(static_cast<uint64_t>(i64))
                                  : NumToString(i64);
  return NoError();
}
...
std::string EnumDef::AllFlags() const {
  ...
  return IsUInt64() ? NumToString(static_cast<uint64_t>(i64))
                    : NumToString(i64);
}
...
CheckedError Parser::ParseEnum(const bool is_union, EnumDef **dest) {
  ...
  if(enum_def->attributes.Lookup("bit_flags") && !IsUnsigned(underlying_type)){
    // todo: Convert to Error in the 1.12!
  ...
  EXPECT('{');
  // A lot of code expects that an enum is not-empty.
  if (is_union || Is('}')) {
  ...
  while(!Is('}')) {
    if (opts.proto_mode && attribute_ == "option") {
      ...
      if (is_union) {
        ...
        if (false == enum_def->uses_multiple_type_instances) {
